...
## des: implement surface water mapping in the QTP region by using trained GMNet.
## !! particularly, this script applicable for 3 cases: 1) both ascending and descending image to be provided.
##           2) only ascending image to be provided. 3) only descending image to be provided.
## how to use:
##   python gmnet_infer.py -as data/tibet_tiles/s1_ascend/*.tif -des data/tibet_tiles/s1_descend/*.tif -s 1 -m $path_model_weights -o $dir_result_save
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=path_gmnet_w_as, 
        help=('pretrained gmnet model (based on ascending image only) weight(torch, .pth)'))

    parser.add_argument(
        '-m_des', '--model_des', type=str, nargs='+', 
        default=path_gmnet_w_des, 
        help=('pretrained gmnet model (based on descending image only) weight(torch, .pth)'))
...
if __name__ == '__main__':
    ...
    path_gmnet_w_as = args.model_as[0]
    path_gmnet_w_des = args.model_des[0]
    odir = args.odir[0]
    ...
    model.load_state_dict(torch.load(path_gmnet_w))
    model_as.load_state_dict(torch.load(path_gmnet_w_as))
    model_des.load_state_dict(torch.load(path_gmnet_w_des))
    if device:
    ...
    for i in range(len(io_files)):
        ...
        print('--- 3. surface water mapping using gmnet model...')
