...
def get_args():
    ...
    parser.add_argument(
            ...
            help='model type of the traiend model, scales/single',
            default=['scales']
            )
    parser.add_argument(
...
'''------ train loops ------'''
def train_loops(model, model_type, loss_fn, optimizer, tra_loader, val_loader, epoches, lr_scheduler=None):
...
if __name__ == '__main__':
    ...
    if model_name == 'unet':
      model = unet(num_bands=num_bands, num_classes=2).to(device)
    elif model_name == 'unet_scales':
      model = unet_scales(num_bands=num_bands, num_classes=2, \
                          scale_high=2048, scale_mid=512, scale_low=256).to(device)
    elif model_name == 'unet_scales_gate':
    ...
    elif model_name == 'deeplabv3plus_mobilev2':
      model = deeplabv3plus_mobilev2(num_bands=num_bands, num_classes=2).to(device)
    elif model_name == 'hrnet':
      model = hrnet(num_bands=num_bands, num_classes=2).to(device)
    print('Model type and name:', model_type + '/' + model_name + '/' + dataset + '/' + s1_orbit + '/' + model_save)
    ...
    # model saving
    path_weights = config.root_proj+'/model/trained_model/'+model_type+'/'+model_name+'/'+dataset+'/'+s1_orbit+'/' + model_save+'_weights.pth'
    torch.save(model.state_dict(), path_weights)
    print('Model weights are saved to --> ', path_weights)
    ## metrics saving
    path_metrics = config.root_proj+'/model/trained_model/'+model_type+'/'+model_name+'/'+dataset+'/'+s1_orbit+'/' + model_save + '_metrics.csv'
