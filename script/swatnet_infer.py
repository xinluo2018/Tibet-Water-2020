...
des:
    implement surface water mapping by using trained watnet
    through funtional api and command line, respectively.
example:
    funtional api:
        water_map = swatnet_infer(rsimg) 
        !!note: the rsimg value is the original value (non-normalizied) or scaled with a integer value, e.g., 100
    command line: 
...
path_swatnet_w_as = ['model/trained_model/gscales/traset/as/model_5_weights.pth']
